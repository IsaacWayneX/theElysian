...
export function Step1CompanyName() {
  const { formData, updateFormData, nextStep } = useMultiStepForm()
  const [value, setValue] = useState(formData.companyName)

  const handleContinue = () => {
    if (value.trim()) {
      updateFormData('companyName', value.trim())
      nextStep()
    }
  }
        ...
        <div className="flex-1 max-w-2xl w-full md:max-w-2xl h-full flex flex-col justify-center">
          <div className="space-y-6 md:space-y-12 overflow-y-auto max-h-full pr-2 py-4 scrollbar-hide">
            ...
            <p className="text-sm sm:text-base md:text-lg lg:text-xl text-gray-300 text-center md:text-left font-bold" style={{ fontFamily: 'var(--font-nohemi)' }}>
              Let's start with the basics. What's the name of your company?
            </p>

            {/* Form */}
            <div className="space-y-4 md:space-y-8">
              <div className="space-y-2 md:space-y-4">
                <Label htmlFor="companyName" className="text-white text-sm sm:text-base md:text-lg lg:text-xl font-bold block text-center md:text-left" style={{ fontFamily: 'var(--font-nohemi)' }}>
                  Company Name
                </Label>
              ...
              <div className="pt-4 md:pt-8">
                <Button
                  ...
                  disabled={!value.trim()}
                  className="w-full bg-white hover:bg-gray-100 text-black px-4 sm:px-12 py-3 sm:py-6 text-sm sm:text-lg md:text-xl lg:text-2xl font-bold flex items-center justify-center gap-2 sm:gap-3 disabled:opacity-50 disabled:cursor-not-allowed rounded-none border-0"
                >
                  Continue
